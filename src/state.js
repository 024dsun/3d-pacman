...
export function setCurrentLevel(l) { currentLevel = l; }
export function setGameOver(g) { gameOver = g; }
export function setIsPaused(p) { isPaused = p; }
export function setGameStarted(g) { gameStarted = g; }
export function setPowerUpActive(p) { powerUpActive = p; }
export function setPowerUpTimer(t) { powerUpTimer = t; }
export function setGhostMultiplier(m) { ghostMultiplier = m; }
export function setGameTime(t) { gameTime = t; }
export function setLastTeleportTime(t) { lastTeleportTime = t; }

export function setCameraMode(m) { cameraMode = m; }
export function setCameraYaw(y) { cameraYaw = y; }
export function setCameraPitch(p) { cameraPitch = p; }
export function setIsPointerLocked(l) { isPointerLocked = l; }

// Array operations
export function clearWalls() { walls.length = 0; }
export function clearGhosts() { ghosts.length = 0; }
export function clearPellets() { pellets.length = 0; }
export function clearPowerUps() { powerUps.length = 0; }
export function clearTeleportZones() { teleportZones.length = 0; }

export function addWall(w) { walls.push(w); }
export function addGhost(g) { ghosts.push(g); }
export function addPellet(p) { pellets.push(p); }
export function addPowerUp(p) { powerUps.push(p); }
export function addTeleportZone(t) { teleportZones.push(t); }

export function removePellet(index) { pellets.splice(index, 1); }
export function removePowerUp(index) { powerUps.splice(index, 1); }
